...
def discover_violations(directory,output):
    ...
    datacount = len(violations.list_weather_violations(directory))
    
    if datacount == None:
        print('No violations found.')
    elif datacount == 1:
        print(str(datacount) + ' violation found.')
    else:
        print(str(datacount) + ' violations found.')

        dataheader = "STUDENT,AIRPLANE,INSTRUCTOR,TAKEOFF,LANDING,FILED,AREA,REASON"
        
    dataheader = list(dataheader.split(","))
     
    for row in data:
        result.append(dataheader)
        result.append(row)
        #strresult = strresult + str(result).strip('[]')
...
def execute(args):
    ...
    if len(args) != 1:
        ...
        tests.test_discover_violations(level)
    else:
        try:
            # Prepare to crash if user gives a non-number
            directory = args[0]
            result = discover_violations(directory,output)
            print(result)
        except:
            print('Usage: python auditor dataset [output.csv]')
