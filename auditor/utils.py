...
def write_csv(data,filename):
    ...
    for row in range(len(data)):
        ...
        for col in range(len(data[row])):
            if data[row][col] == datetime:    
                data[row][col] = datetime.data[row][col]
...
def daytime(time,daycycle):
    ...
    dYear = str(time.strftime('%Y'))
    dMonDay = str(time.strftime('%m-%d'))
    mk = ''
    m = ''
    MD = ''
    sunset = ''
    ...
    for k in daycycle:
        ...
        if dYear == k:
            mk = k
            sunrise = daycycle[k][dMonDay]['sunrise']
            sunset = daycycle[k][dMonDay]['sunset']
            for m in daycycle[k].keys():
                if m == dMonDay:
                    MD = m
       
    sunrisetz = str_to_time(mk+'-'+MD+' '+sunrise, daycycle["timezone"])
    sunsettz = str_to_time(mk+'-'+MD+' '+sunset, daycycle["timezone"])
    if time.tzinfo is None or time.tzinfo == None:
        time = time.replace(tzinfo=tz)
        if time > sunrisetz and time < sunsettz:
            return True
        else:
            return False
    else:
