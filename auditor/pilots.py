...
has the following header:
    ...
    ID  LASTNAME  FIRSTNAME  JOINED  SOLO  LICENSE  50 HOURS  Z  ADVANCED  MULTIENGINE
...
def get_certification(takeoff,student):
    ...
    response = PILOT_INVALID
    third = utils.str_to_time(student[3],takeoff.tzinfo)
    if (third) and takeoff < third:
        #print("inside third is " , third )
        #print("inside third -takeoff is " , takeoff )
        response = PILOT_INVALID
    fourth = utils.str_to_time(student[4],takeoff.tzinfo)
    if (third) and takeoff > third:
        #print("inside third PILOT_NOVICE >>  is " , third )
        #print("inside third -takeoff is " , takeoff )
        response = PILOT_NOVICE
    fifth = utils.str_to_time(student[5],takeoff.tzinfo)
    sixth = utils.str_to_time(student[6],takeoff.tzinfo)
    if (fourth) and takeoff > fourth:
        #print("inside fourth PILOT_STUDENT >>  is " , fourth )
        #print("inside fourth -takeoff is " , takeoff )
        response = PILOT_STUDENT
    if (fifth) and takeoff > fifth:
        #print("inside fifth PILOT_CERTIFIED >>  is " , fifth )
        #print("inside fifth -takeoff is " , takeoff )
        response = PILOT_CERTIFIED
    if  (sixth) and takeoff > sixth:
        #print("inside sixth PILOT_50_HOURS >>  is " , sixth )
        #print("inside sixth -takeoff is " , takeoff )
        response = PILOT_50_HOURS
    return response
...
def has_instrument_rating(takeoff,student):
    ...
    from dateutil.parser import parse
    response = True
    seventh = utils.str_to_time(student[7],takeoff.tzinfo)
    if (seventh) and seventh < takeoff:
...
def has_advanced_endorsement(takeoff,student):
    ...
    from dateutil.parser import parse
    response = True
    seventh = utils.str_to_time(student[8],takeoff.tzinfo)
    if (seventh) and seventh < takeoff:
...
def has_multiengine_endorsement(takeoff,student):
    ...
    from dateutil.parser import parse
    response = True
    seventh = utils.str_to_time(student[9],takeoff.tzinfo)
    if (seventh) and seventh < takeoff:
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    for row in minimums[1:]:
        ...
        if col0 and col1 and col2 and col3:
            matches.append(list(map(float,row[-4:])))
